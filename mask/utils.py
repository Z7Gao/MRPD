# Authors' notes: 
### This file is heavily built on Score-MRI's 
### https://github.com/HJ-harry/score-MRI/blob/main/utils.py, 
### which has claimed to refer to FastMRI's codebase in its paper [1].
### [1] Chung, Hyungjin, and Jong Chul Ye. "Score-based diffusion models for accelerated MRI." Medical image analysis 80 (2022): 102479.
...
def get_mask(img, size, batch_size, type='gaussian2d', acc_factor=8, center_fraction=0.04, fix=False):
  ...
  if type == 'gaussian2d':
    ...
    if fix:
      ...
      samples = np.random.normal(loc=mean, scale=std, size=int(Nsamp))
      int_samples = samples.astype(int)
      int_samples = np.clip(int_samples, 0, size - 1)
      mask[... , int_samples] = 1
